...
PassportConfigurator.prototype.configureProvider = function(name, options) {
    ...
    case 'ldap':
      ...
      function(req, user, done) {
        if (user) {
          ...
          self.userIdentityModel.login(req, provider, authScheme, profile, {},
    ...
    case 'saml':
      ...
      function(req, profile, done) {
        ...
        if (link) {
          ...
          self.userIdentityModel.login(req, name, authScheme, profile, {},
            options, loginCallback(req, done));
        }
      }
      );
  ...
  if (authType === 'local') {
    ...
    self.app.get(authPath, (req, res, next) => {
      const { returnTo, failureTo, userId } = req.query
      req.session.returnTo = returnTo
      req.session.failureTo = failureTo
      if (req.accessToken) {
        req.session.userId = req.accessToken.userId.toString()
      }
      const stateObject = {}
      const state = Buffer.from(JSON.stringify(stateObject)).toString('base64')
      const authenticator = passport.authenticate(name, _.defaults({
        scope: scope,
        session: session,
        state: state,
      }, options.authOptions))
